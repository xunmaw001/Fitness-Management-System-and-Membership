...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jianshenleixing');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','jianshenleixing');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('jianshenleixing', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussjianshenleixing/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('jianshenleixing', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('jianshenleixing', this.detail);
				this.$utils.msg('审核成功');
