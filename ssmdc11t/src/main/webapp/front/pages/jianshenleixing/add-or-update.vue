	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jianshenleixing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																																												if(this.ruleForm.id){
					await this.$api.update(`jianshenleixing`, this.ruleForm);
				}else{
					await this.$api.add(`jianshenleixing`, this.ruleForm);
				}
